...
export default function MySQLSampleData() {
  ...
  const insertSQL = useMemo(
    () => `-- SummitCare — Sample Data Inserts (MySQL 8.0)
-- IMPORTANT:
-- 1) Replace USER_EMAIL with your actual login email (same one used in the app).
...
-- User-owned medications (replace USER_EMAIL)
INSERT INTO medications (
  created_by_email, name, dosage, indication, start_date, notes, altitude_risk_level, medication_database_id
) VALUES
  ('USER_EMAIL', 'Acetazolamide', '125mg twice daily', 'AMS prophylaxis', '2025-06-01', 'Demo seed', 'moderate',
    (SELECT id FROM medication_database WHERE name = 'Acetazolamide')),
  ('USER_EMAIL', 'Ibuprofen', '400mg as needed', 'Pain relief', '2025-05-15', 'Demo seed', 'low',
    (SELECT id FROM medication_database WHERE name = 'Ibuprofen')),
  ('USER_EMAIL', 'Albuterol', '2 puffs as needed', 'Bronchospasm', '2025-05-10', 'Demo seed', 'moderate',
    (SELECT id FROM medication_database WHERE name = 'Albuterol'));

-- Sample climbs (replace USER_EMAIL)
INSERT INTO climbs (
  ...
  ('USER_EMAIL', 'Mount Whitney', 14505, 'Sierra Nevada, California', '2025-07-15', 2,
  ...
  ('USER_EMAIL', 'Pikes Peak', 14115, 'Front Range, Colorado', '2025-08-12', 1,
...
SELECT id, 'First Aid Kit', 'health', 1, 1, 0, 'critical', 0.25, '' FROM climbs WHERE mountain_name = 'Mount Whitney' AND created_by_email = 'USER_EMAIL';
INSERT INTO climb_gear_items (climb_id, item_name, category, quantity, required, packed, importance, estimated_weight_kg, notes)
SELECT id, 'Water (3L)', 'food_water', 1, 1, 0, 'critical', 3.00, 'Hydration system or bottles' FROM climbs WHERE mountain_name = 'Mount Whitney' AND created_by_email = 'USER_EMAIL';
INSERT INTO climb_gear_items (climb_id, item_name, category, quantity, required, packed, importance, estimated_weight_kg, notes)
SELECT id, 'Headlamp', 'technical', 1, 1, 0, 'high', 0.10, 'With spare batteries' FROM climbs WHERE mountain_name = 'Mount Whitney' AND created_by_email = 'USER_EMAIL';
INSERT INTO climb_gear_items (climb_id, item_name, category, quantity, required, packed, importance, estimated_weight_kg, notes)
SELECT id, 'Insulating Layer', 'clothing', 1, 1, 0, 'high', 0.40, 'Fleece or puffy' FROM climbs WHERE mountain_name = 'Mount Whitney' AND created_by_email = 'USER_EMAIL';
INSERT INTO climb_gear_items (climb_id, item_name, category, quantity, required, packed, importance, estimated_weight_kg, notes)
SELECT id, 'Microspikes', 'technical', 1, 0, 0, 'recommended', 0.40, '' FROM climbs WHERE mountain_name = 'Mount Whitney' AND created_by_email = 'USER_EMAIL';

-- Gear for Pikes Peak
INSERT INTO climb_gear_items (climb_id, item_name, category, quantity, required, packed, importance, estimated_weight_kg, notes)
SELECT id, 'Map & Compass or GPS', 'navigation', 1, 1, 0, 'critical', 0.15, '' FROM climbs WHERE mountain_name = 'Pikes Peak' AND created_by_email = 'USER_EMAIL';
INSERT INTO climb_gear_items (climb_id, item_name, category, quantity, required, packed, importance, estimated_weight_kg, notes)
SELECT id, 'Shell (Jacket)', 'clothing', 1, 1, 0, 'high', 0.35, 'Water/wind resistant' FROM climbs WHERE mountain_name = 'Pikes Peak' AND created_by_email = 'USER_EMAIL';
INSERT INTO climb_gear_items (climb_id, item_name, category, quantity, required, packed, importance, estimated_weight_kg, notes)
SELECT id, 'Trekking Poles', 'technical', 1, 0, 0, 'optional', 0.60, '' FROM climbs WHERE mountain_name = 'Pikes Peak' AND created_by_email = 'USER_EMAIL';

COMMIT;

-- Done. Replace USER_EMAIL before running.
`,
    []
  );
      ...
      <Card className='mb-4'>
        <CardHeader className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
          <div>
            ...
            <CardDescription>
              Copy or download SQL to seed demo data into your MySQL database.
            </CardDescription>
          </div>
        ...
        <CardContent>
          <div className='text-sm text-slate-600 mb-3'>
            Tip: Replace USER_EMAIL with your app account email before running.
          </div>
