...
import { Bot, Loader2, Send, User } from '@/components/icons';
...
export default function AIPlayground() {
  ...
  const [system, setSystem] = useState(
    'You are a helpful mountaineering and health information assistant. Avoid medical advice; recommend consulting a physician.'
  );
  const [model, setModel] = useState('gpt-4o-mini');
  const [isLoading, setIsLoading] = useState(false);

  // Backoff handling to avoid repeated 429s
  ...
  // Manual user toggle (default off if backoff active)
  ...
  const setBackoffForHours = (hours) => {
    ...
    setUseOpenAI(false); // Force switch off when backoff is active
  };
  ...
  const clearBackoff = () => {
    ...
    setBackoffUntil(0);
    // Do NOT set setUseOpenAI(true) here. Let the user explicitly toggle it back on.
  };
  ...
  const onSend = async () => {
    ...
    try {
      if (useOpenAI && !backoffActive) {
        // Primary: OpenAI via backend function
        ...
        // Skip OpenAI during backoff or when user toggled off, proceed directly to fallback
        replyText = await runFallback();
      }
    } catch (e) {
      const status = e?.response?.status || e?.status || 0;
      // Check for common error message properties for OpenAI
      const message = e?.response?.data?.error?.message || e?.message || '';
      const isQuota = status === 429 || /quota|rate limit|429/i.test(String(message));

      if (isQuota) {
        // Set 12-hour backoff window to prevent repeated errors
        setBackoffForHours(12);
      }

      try {
        // Always attempt fallback if OpenAI failed or was skipped
        replyText = await runFallback();
      } catch {
      ...
      setMessages([
        ...next,
        { role: 'assistant', content: replyText || 'Sorry, I’m having trouble right now.' },
      ]);
      setIsLoading(false);
    }
  };
      ...
      <div className='max-w-5xl mx-auto space-y-6'>
        <div className='flex items-start justify-between gap-4'>
          <div>
            ...
            <p className='text-text-secondary mt-1'>
              Chat with OpenAI (gpt-4o / gpt-4o-mini). This is for demonstration only.
            </p>
          </div>
        ...
        {backoffActive && (
          <Alert variant='destructive' className='bg-red-50'>
            ...
            <AlertDescription>
              We&apos;ll automatically use the built-in model instead. Backoff ends at{' '}
              {new Date(backoffUntil).toLocaleString()}.
            </AlertDescription>
          </Alert>
        )}
        ...
        <Card className='alpine-card border-0 shadow-lg'>
          ...
          <CardContent className='p-6 space-y-4'>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              <div>
                ...
                <Select
                  ...
                  disabled={!useOpenAI || backoffActive}
                >
                ...
                {(!useOpenAI || backoffActive) && (
                  <p className='text-xs text-text-secondary mt-2'>
                    Using built-in model while OpenAI is off or in backoff.
                  </p>
                )}
              </div>
              ...
              <div className='md:col-span-2'>
                ...
                <Input
                  value={system}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSystem(e.target.value)}
            ...
            <div className='flex items-center justify-between p-3 rounded-lg border border-stone-200 bg-white'>
              ...
              <Switch
                checked={useOpenAI && !backoffActive} // Only checked if user wants OpenAI AND no backoff
                onCheckedChange={(checked) => {
                  if (backoffActive && checked) {
                    // If backoff is active, prevent turning on OpenAI
                    return;
                  }
                  if (!checked) {
                    // If user manually turns it OFF, clear any active backoff.
                    // This allows them to manually override/clear a backoff period.
                    clearBackoff();
                  }
                  setUseOpenAI(checked); // Update the user's preference
                }}
                disabled={backoffActive} // Disable switch if backoff is active
              />
            </div>

            <div className='text-xs text-text-secondary'>
              Note: Do not use this for real medical decisions. Always consult a qualified
              physician.
            </div>
          </CardContent>
        </Card>
            ...
            <div className='border-t border-stone-100 p-4 md:p-6'>
              <div className='flex items-end gap-3'>
                ...
                <Button
                  ...
                  className='mountain-gradient hover:opacity-90 transition-opacity min-w-[110px]'
                >
                  {isLoading ? (
                    <>
