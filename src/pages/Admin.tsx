        ...
        {delta && (
          <div
            className={cls(
              ...
              trend === "up" && "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200",
              trend === "down" && "bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-200",
              trend === "flat" && "bg-stone-100 text-stone-700 dark:bg-stone-900 dark:text-stone-200"
            )}
          >
            <TrendingUp className={cls("h-3 w-3", trend === "down" && "rotate-180", trend === "flat" && "rotate-90")} />
            {delta}
          </div>
        )}
      </div>
...
export default function Admin() {
  ...
  const recent = [
    { id: "evt_7f21", type: "Login", user: "alex@summitcare.app", at: "2025-10-14 22:13", meta: "2FA success" },
    { id: "evt_7f1c", type: "Policy Update", user: "megan@summitcare.app", at: "2025-10-14 20:02", meta: "RBAC: analyst" },
    { id: "evt_7e90", type: "DB Migration", user: "deploy@vercel", at: "2025-10-14 18:44", meta: "schema v12 → v13" },
    { id: "evt_7e51", type: "Job Retry", user: "system", at: "2025-10-14 12:10", meta: "getAeTrendsCached" },
  ];
  const services = [
  ...
  return (
    <div className="mx-auto max-w-7xl p-4 md:p-6">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-stone-900 dark:text-stone-100">Admin Console</h1>
          <p className="text-sm text-stone-500">Operational overview, system health, and administrative tools.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat icon={Users} label="Total Users" value={metrics.users.toLocaleString()} delta="+4.2%" trend="up" />
        <Stat icon={Activity} label="Active Today" value={metrics.activeToday.toLocaleString()} delta="-1.8%" trend="down" />
        <Stat icon={FileCog} label="Jobs Queued" value={String(metrics.jobsQueued)} delta="~" trend="flat" />
        <Stat icon={AlertTriangle} label="Incidents (30d)" value={String(metrics.incidents30d)} delta="-1" trend="up" />
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card>
            ...
            <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-3">
              {services.map((s) => (
                <div key={s.name} className="flex items-center gap-3 rounded-xl border border-stone-100 p-3 dark:border-stone-900">
        ...
        <div className="space-y-4">
          <Card>
            ...
            <div className="grid grid-cols-1 gap-2 p-4">
              <AdminAction label="RBAC Policies" icon={KeyRound} onClick={() => alert("Open RBAC policies")} />
              <AdminAction label="Database Console" icon={Database} onClick={() => (window.location.href = "/database")} />
              <AdminAction label="Job Runner" icon={FileCog} onClick={() => alert("Open job runner")} />
              <AdminAction label="System Settings" icon={Settings} onClick={() => alert("Open settings")} />
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-3 p-4 text-sm">
              ...
              <p className="rounded-xl bg-emerald-50 p-3 text-emerald-900 dark:bg-emerald-900/20 dark:text-emerald-200">
                Cache warmers are stable; Base44 functions returning <code>200</code> within 220–280ms p95.
              </p>
            </div>
          </Card>
        </div>
