...
type OverallRisk = 'severe' | 'high' | 'moderate' | 'low' | 'minimal';

export interface MedicationLike {
  id?: number | string;
  name?: string;
  // add fields here as your UI needs them
}

type SvgIcon = React.ComponentType<React.SVGProps<SVGSVGElement>>;

interface Recommendation {
  type: 'warning' | 'caution' | 'info' | 'general';
  title: string;
  message: string;
  icon: SvgIcon;
...
export interface AltitudeInsightsProps {
  medications: MedicationLike[];
  overallRisk: OverallRisk;
}

export default function AltitudeInsights({ medications, overallRisk }: AltitudeInsightsProps) {
  const getAltitudeRecommendations = React.useCallback((): Recommendation[] => {
  ...
  }, [overallRisk, medications]);

  const elevationGuidance = React.useMemo(() => {
    switch (overallRisk) {
      case 'severe':
        return {
          max: '8,000ft',
  ...
  return (
    <div className='space-y-6'>
      <Card className='alpine-card border-0 shadow-lg'>
        ...
        <CardContent className='p-6'>
          <div className='text-center mb-4'>
            <div className='text-3xl font-bold text-text-primary mb-2'>{elevationGuidance.max}</div>
            ...
            {getAltitudeRecommendations().map((rec, index) => {
              ...
              return (
                <div key={index} className='flex items-start gap-3 p-3 rounded-lg bg-stone-50'>
                  ...
                  <div>
                    <h4 className='font-medium text-text-primary text-sm mb-1'>{rec.title}</h4>
      ...
      <Card className='alpine-card border-0 shadow-lg'>
        <CardHeader className='border-b border-stone-100'>
          <CardTitle className='text-lg font-bold text-text-primary'>Emergency Contacts</CardTitle>
        </CardHeader>
        <CardContent className='p-6'>
