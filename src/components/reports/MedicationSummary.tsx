...
import { Pill, Calendar } from '@/components/icons';
        ...
        {medications.length === 0 ? (
          ...
          <div className='space-y-4'>
            {medications.map((medication) => (
              <div key={medication.id} className='p-4 rounded-xl border border-stone-100 bg-white'>
                <div className='flex justify-between items-start mb-3'>
                  <div>
                    <h3 className='font-semibold text-text-primary text-lg'>{medication.name}</h3>
                    <p className='text-text-secondary'>{medication.dosage}</p>
                  </div>
                  {medication.altitude_risk_level && (
                    <Badge
                      className={`${getRiskColor(medication.altitude_risk_level)} border font-medium flex items-center gap-1`}
                    >
