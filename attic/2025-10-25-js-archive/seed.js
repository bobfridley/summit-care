...
export default async function handler(req, res) {
  ...
  const token =
    req.headers['x-admin-token'] || new URL(req.url, 'http://x').searchParams.get('token') || null;
  ...
  try {
    ...
    await withConn(async (conn) => {
      if (doContra) {
        const rows = [
          {
            contraindication: 'MAOIs with SSRIs',
            level: 'major',
            note: 'Risk of serotonin syndrome',
          },
          {
            contraindication: 'Tyramine-rich foods',
            level: 'moderate',
            note: 'Hypertensive crisis risk',
          },
          {
            contraindication: 'Pseudoephredrine',
            level: 'moderate',
            note: 'May elevate blood pressure',
          },
        ];
        for (const r of rows) {
      ...
      if (doAe) {
        ...
        const day = (d) =>
          new Date(today.getTime() - d * 24 * 3600 * 1000).toISOString().slice(0, 10);
        const buckets = [
          { date: day(0), count: 12 },
          { date: day(1), count: 9 },
          { date: day(2), count: 7 },
