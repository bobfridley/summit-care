...
async function getPool() {
  ...
  const sslMode = (Deno.env.get('MYSQL_SSL_MODE') || 'insecure').toLowerCase();
  ...
  const createInsecurePool = () => mysql.createPool({ ...common, ssl: { rejectUnauthorized: false, minVersion: 'TLSv1.2' } });
  const createPlainPool = () => mysql.createPool({ ...common });

  if (disableSsl) {
    const plain = createPlainPool();
    const conn = await plain.getConnection();
