...
export const climbGearItems = mysqlTable(
  'climb_gear_items',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    climbId: bigint('climb_id', { mode: 'number', unsigned: true })
      .notNull()
      .references(() => userClimbs.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
    ...
    importance: mysqlEnum(['critical', 'high', 'recommended', 'optional'])
      .default('recommended')
      .notNull(),
...
export const climbMedications = mysqlTable(
  'climb_medications',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    climbId: bigint('climb_id', { mode: 'number', unsigned: true })
    ...
    medicationId: bigint('medication_id', { mode: 'number', unsigned: true })
      .notNull()
      .references(() => userMedications.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
...
export const contraindications = mysqlTable(
  'contraindications',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    drug: varchar({ length: 128 }).notNull(),
  ...
  (table) => [primaryKey({ columns: [table.id], name: 'contraindications_id' })]
);

export const gearCatalog = mysqlTable(
  'gear_catalog',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    itemName: varchar('item_name', { length: 255 }).notNull(),
    ...
    defaultImportance: mysqlEnum('default_importance', [
      'critical',
      'high',
      'recommended',
      'optional',
...
export const gearPresetItems = mysqlTable(
  'gear_preset_items',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    gearPresetId: bigint('gear_preset_id', { mode: 'number', unsigned: true })
      .notNull()
      .references(() => gearPresets.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
    gearCatalogId: bigint('gear_catalog_id', { mode: 'number', unsigned: true }).references(
      () => gearCatalog.id,
...
export const gearPresets = mysqlTable(
  'gear_presets',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
...
export const medicationAltitudeEffects = mysqlTable(
  'medication_altitude_effects',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    medicationDatabaseId: bigint('medication_database_id', { mode: 'number', unsigned: true })
      .notNull()
      .references(() => medicationDatabase.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
...
export const medicationDatabase = mysqlTable(
  'medication_database',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
...
export const medicationWarnings = mysqlTable(
  'medication_warnings',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    medicationDatabaseId: bigint('medication_database_id', { mode: 'number', unsigned: true })
...
export const userClimbStatusHistory = mysqlTable(
  'user_climb_status_history',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    climbId: bigint('climb_id', { mode: 'number', unsigned: true })
      .notNull()
...
export const userClimbs = mysqlTable(
  'user_climbs',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    ...
    difficultyLevel: mysqlEnum('difficulty_level', [
      ...
      .notNull(),
    climbingStyle: mysqlEnum('climbing_style', [
      'day_hike',
      'overnight',
      'multi_day',
      'expedition',
    ...
    status: mysqlEnum(['planning', 'confirmed', 'in_progress', 'completed', 'cancelled'])
      .default('planning')
      .notNull(),
    notes: text(),
    createdAt: timestamp('created_at', { mode: 'string' }).defaultNow().notNull(),
...
export const userMedications = mysqlTable(
  'user_medications',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    createdByEmail: varchar('created_by_email', { length: 255 }).notNull(),
    ...
    notes: text(),
    altitudeRiskLevel: mysqlEnum('altitude_risk_level', [
      'low',
      'moderate',
      'high',
      'severe',
    ]).notNull(),
    medicationDatabaseId: bigint('medication_database_id', {
      mode: 'number',
      unsigned: true,
    }).references(() => medicationDatabase.id, { onDelete: 'set null', onUpdate: 'cascade' }),
    createdAt: timestamp('created_at', { mode: 'string' }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { mode: 'string' }).defaultNow().onUpdateNow().notNull(),
  },
  (table) => [
...
export const userTripReports = mysqlTable(
  'user_trip_reports',
  {
    id: bigint({ mode: 'number', unsigned: true }).autoincrement().notNull(),
    createdByEmail: varchar('created_by_email', { length: 255 }).notNull(),
    climbId: bigint('climb_id', { mode: 'number', unsigned: true }).references(
      () => userClimbs.id,
      { onDelete: 'set null', onUpdate: 'cascade' }
    ),
    name: varchar({ length: 255 }).notNull(),
